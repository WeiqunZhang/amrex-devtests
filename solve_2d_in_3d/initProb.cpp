...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(solution, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        {
            actual_init_poisson(i,j,k,rhsfab,solfab,prob_lo,prob_hi,dx,hd);
        });
    }

    MultiFab::Copy(exact_solution, solution, 0, 0, 1, solution.nGrowVect());
    solution.setVal(0.0, 0, 1, 0);
}
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(rhs, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            auto rhsfab = rhs.array(mfi);
            auto solnfab = solution.array(mfi);
            auto exactfab = exact_solution.array(mfi);
            auto acoeffab = acoef.array(mfi);
            auto bcoeffab = bcoef.array(mfi);
        ...
        solution.setVal(0.0,0,1,0); // set interior to 0
    }
#endif
}
