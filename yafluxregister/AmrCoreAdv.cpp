...
AmrCoreAdv::Evolve ()
{
    ...
    for (int step = istep[0]; step < max_step && cur_time < stop_time; ++step)
    {
        ...
        amrex::Print().SetPrecision(17) << "Coarse STEP " << step+1 << " ends." << " TIME = " << cur_time
                                    ...
                                    const DistributionMapping& dm)
{
    ...
    if (lev > 0 && do_reflux) {
        flux_reg[lev].reset(new YAFluxRegister(ba, boxArray(lev-1), dm, DistributionMap(lev-1),
                                               Geom(lev), Geom(lev-1), refRatio(lev-1),
                                               lev, ncomp));
    }
...
AmrCoreAdv::timeStepWithSubcycling (int lev, Real time, int iteration)
{
    ...
    if (lev < finest_level)
    {
        ...
        if (do_reflux)
        {
            ...
            flux_reg[lev+1]->Reflux(phi_new[lev]);
        }
...
AmrCoreAdv::ReadCheckpointFile ()
{
    ...
    for (int lev = 0; lev <= finest_level; ++lev) {
        ...
        // build MultiFab and YAFluxRegister data
        int ncomp = 1;
        int nghost = 0;
